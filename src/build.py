...
def _intern_strings(SCRIPTS):
    ...
    for key in SPRITESHEETS.TILEMAPS.keys():
        if key not in UNIQUE_STRINGS:
            UNIQUE_STRINGS.append(key)
        for idx in SPRITESHEETS.TILEMAPS[key]:
            name = SPRITESHEETS.TILEMAPS[key][idx]
            if name not in UNIQUE_STRINGS:
                UNIQUE_STRINGS.append(name)
    ...
    if "tileset" not in UNIQUE_STRINGS:
        UNIQUE_STRINGS.append("tileset")
...
#define _TILESET {UNIQUE_STRINGS.index("tileset")}
...
def _intern_floats(SCRIPTS): 
    ...
    float_data_dot_c = """// Generated by Red Pants Engine
#ifndef FLOAT_DATA_LOAD
#include "floatdata.h"
#endif
const float FLOATS[NUM_FLOATS] = {\n  """ + ",\n  ".join(str(v) for v in UNIQUE_FLOATS) + "\n};"
    float_data_dot_h = f"""// Generated by Red Pants Engine
#ifndef FLOAT_DATA_LOAD
#define FLOAT_DATA_LOAD 1
#define NUM_FLOATS {len(UNIQUE_FLOATS)}
extern const float FLOATS[{len(UNIQUE_FLOATS)}];
#endif
"""
...
#define SCRIPT_MAPS_SIZE {len(scriptmaps)}
extern int SCRIPTS[];
...
"""
    ...
    for key in SPRITESHEETS.TILEMAPS:
        if "\\" in key: continue
        tilemap = SPRITESHEETS.TILEMAPS[key]
        spritesheets_dot_c += f"    add_tile_map({UNIQUE_STRINGS.index(key)}, {UNIQUE_STRINGS.index(tilemap['00'])},  {UNIQUE_STRINGS.index(tilemap['01'])},  {UNIQUE_STRINGS.index(tilemap['02'])}, {UNIQUE_STRINGS.index(tilemap['10'])},  {UNIQUE_STRINGS.index(tilemap['11'])},  {UNIQUE_STRINGS.index(tilemap['12'])}, {UNIQUE_STRINGS.index(tilemap['20'])},  {UNIQUE_STRINGS.index(tilemap['21'])},  {UNIQUE_STRINGS.index(tilemap['22'])});\n"
    spritesheets_dot_c += "}\n"
...
void world_load() """ + "{\n"
    ...
    worlddata_dot_c += "}\n"
    worlddata_dot_h = f"""// The following code was generated by the Red Pants Engine
// lets go baby
...
def make_image_data():
    imagedata_dot_h = _convert_images("-b" in sys.argv or "--build" in sys.argv)
    with open(C_PATH / "imagedata.h", "w+") as f:
        f.write(imagedata_dot_h)
    
def _convert_images(need_mode=True):
    imagedata_dot_h = """
// The following code was generated by the Red Pants Engine
// lets go baby
...
#define NUM_TILEMAPS """ + str(len(SPRITESHEETS.TILEMAPS)) + """
"""
    if need_mode:
